...
////////////add   new join file
int inner_join(string input,unordered_map<int, int> &map){//default value is id
...
*/
	std::vector< pair<int,int> > output_vector;
	

	//read in second

	int gender,id,age;
	istringstream second_file(input);
    string line;
    while(second_file >> gender >> id)//problem
    {
		if( map.find(id) != map.end()){
			output_vector.push_back( std::make_pair(gender,map[id]) );
		}
	}
	ofstream outfile;
	outfile.open("output.txt", ios::trunc);
	for(auto inner = output_vector.begin(); inner != output_vector.end(); inner++)
	{
		//cout<<"enter"<<endl;
		outfile << inner -> first << " " << inner -> second  << endl;
	}

	//std::ifstream second_file(gender_file);
	...
	//read all of the file, put them into vector of vectors.
	//make a hash table to put in

	//read from another file, check the hash table if it exists in the table, join the crresponding results and put into the third vector.
}


int input_file(string d_file, unordered_map<int, int> &map){
	//read in first
	std::ifstream infile(d_file);
	int age,id,gender;
	if(!infile.is_open())
	{
		return -1;
...
string receivePutRequest(int sockfd, char* buf, uint32_t len, std::string& sender)
{
	...
	input_file("age_file.txt", age_map);
