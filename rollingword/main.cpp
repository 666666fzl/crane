...
void processPutRequestThread()
{
    ...
    while(true)
    {
        ...
        createFilename = receivePutRequest(listenFileSocket, buf, 1024, sender);
        cout<<"FINIGHING PUTTING REQUEST "<<createFilename<<endl;

        if(createFilename.find("file_location_log.txt")!=string::npos)
        {
            continue;
        }
        //cout<<createFilename<<endl;
        // unordered_map<string, int> countMap;
        // wordCounter(createFilename, countMap);
        ...
        putFileRequest(createFilename+"Result", createFilename+"Result", singleNode);
    }
}
...
// {
//     listenStreamSocket = open_socket(port + 5); 
//     char buf[1024];
//     while(true)
//     {
//         int finishStreamSocket = listen_socket(listenStreamSocket);
//         receiveStreamRequest(listenDeleteFileSocket); 
//     }
// }

void sendWordCountFile(string filename, vector<Node> group)
{
    char command_data[1024];
    bzero(command_data, sizeof(command_data));
    sprintf(command_data, "split -n %d %s", group.size(), filename.c_str());
    //string command_data = "split -n " + to_string(group.size()) + filename;
    ...
    if(parent){
        ...
        {
            string splitFilename = "xa";
            splitFilename.push_back(static_cast<char>('a'+i));
            cout<<splitFilename<<endl;
            putFileHelper(splitFilename, splitFilename, group[i].ip_str);
        }
...
void listeningCin()
{
    ...
    while (true)
    {
        ...
        else if (tokens[0].compare("wordcount") == 0)
        {
            sendWordCountFile(tokens[1], members);   
        }
